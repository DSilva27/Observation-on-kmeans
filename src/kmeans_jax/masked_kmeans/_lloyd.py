...
def _kmeans_step(
    ...
    labels = assign_clusters(centroids, data, masks)
    centroids = compute_centroids(data, masks, labels, centroids.shape[0])
    # loss = loss.at[counter].set(compute_loss(data, centroids, labels))
    loss = compute_loss(data, masks, centroids, labels)
